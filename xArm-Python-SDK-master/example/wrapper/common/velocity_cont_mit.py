...
# Nullspace P gain.
Kn = np.asarray([40.0, 40.0, 40.0, 40.0, 5.0, 5.0, 5.0])
...
def calculate_jacobian(model, data, joint_angles, actuator_ids, dof_ids, site_id, tolerance=0.01, max_steps=100):
    jac = np.zeros((6, model.nv))
    model.opt.timestep = 0.2  # Increase the timestep for faster convergence

    time_start = time.time()
    step_count = 0
    while True:
        step_start = time.time()
        data.ctrl[:7] = joint_angles[:7]
        mujoco.mj_step(model, data)
        step_count += 1

        # Check for convergence
        current_joint_angles = data.qpos[dof_ids][:7]
        if np.allclose(current_joint_angles, joint_angles[:7], atol=tolerance):
        ...
        if (time.time() - time_start > 0.2) or (step_count >= max_steps):
            print(f"Max steps reached: {step_count}, Current angles: {current_joint_angles}")
            break
