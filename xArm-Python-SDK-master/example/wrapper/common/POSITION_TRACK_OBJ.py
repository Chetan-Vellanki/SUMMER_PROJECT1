...
def read_waypoints(file_path, waypoints) : 
    with open(file_path, 'r') as file:
        ...
        if len(coords) == 3:
            ...
            print("Error: File should contain exactly three comma-separated values.")
    return waypoints
...
def main() -> None:
    ...
    waypoints = read_waypoints(file_path, waypoints)
    ...
    while True:
        ...
        if(reach_flag == 0) : 
            current_waypoint = [waypoints[0] - 70 ,waypoints[1],waypoints[2] - 90]
        # current_waypoint=[]
        if(reach_flag == 1) : 
            if(waypoint[0] >= 650) :
                arm.disconnect()
            Kp_reach = 1.5
            current_waypoint = [waypoints[0] ,waypoints[1],waypoints[2] - 90]
        ...
        if current_waypoint_index >= len(waypoints):
            ...
            code = arm.set_gripper_position(20, wait=True, auto_enable= True)
            print(code)
            print(waypoints)
            # current_waypoint = [waypoints[0], waypoints[1], 280]
            waypoints = read_waypoints(file_path, waypoints)
            arm.motion_enable(enable=True)
            arm.set_mode(0)
            arm.set_state(state=0)
        ...
        if(reach_flag == 0) : 
            target_velocity = desired_velocity + Kp * error
        if(reach_flag == 1) : 
            target_velocity = desired_velocity + Kp_reach * error
        print("TARGET VELOCITY = ", target_velocity)
        previous_error = error
        previous_time = current_time
        twist[:3] = Kpos * target_velocity / integration_dt
