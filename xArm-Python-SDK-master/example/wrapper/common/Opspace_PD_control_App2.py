...
arm.reset(wait=True)
arm.set_position(x=290, y=0, z=120, roll=-180, pitch=0, yaw=0, speed=20, is_radian=False, wait=True)
# Cartesian impedance control gains.
impedance_pos = np.asarray([1, 1, 1])  # [N/m]
impedance_ori = np.asarray([1 , 1, 1])  # [Nm/rad]

# Joint impedance control gains.
Kp_null = np.asarray([0.0001, 0.0001, 0.0001, 0.0001, 0.0001, 0.0001, 0.0001])
...
def convert_joint_torques_to_joint_Velocities(model , data , joint_angles , actuator_ids, dof_ids, site_id, gen_torques, joint_acc, curr_joint_vel,M_inv ) :
    ...
    #         difference_in_position = np.array(data.qpos[dof_ids] - joint_angles)
    #         print("Difference in position :", difference_in_position)
    #         M_inv = M_inv[ : 7, :7] 
    #         C = np.zeros(model.nv)
    #         mujoco.mj_rne(model, data, 0, C)
    #         time_set = 0.3
    #         # new_joint_accelerations = np.array(M_inv @ (gen_torques - C[ : 7] @ curr_joint_vel))
    #         # new_joint_velocities = np.array(curr_joint_vel) + new_joint_accelerations * 3
    #         # Calculate M
    #         if abs(np.linalg.det(M_inv)) >= 1e-2:
    #             M = np.linalg.inv(M_inv)
    #         else:
    #             M = np.linalg.pinv(M_inv, rcond=1e-2)
    #         beta = np.array(M) # + np.array(C) * time_set 

    #         if abs(np.linalg.det(beta)) >= 1e-2:
    #             beta_inv = np.linalg.inv(beta)
    #         else:
    #             beta_inv = np.linalg.pinv(beta, rcond=1e-2)

    #         new_joint_velocities = beta_inv @ (np.array(gen_torques * time_set) + np.array(M @ curr_joint_vel))
    #         break
    data.qpos[dof_ids] = joint_angles
    mujoco.mj_step(model, data)
    # data.qpos[ : 7] = joint_angles - difference_in_position
    # mujoco.mj_step(model, data)
    data.qvel[ : 7] = curr_joint_vel
    difference_in_position = np.array(data.qpos[dof_ids] - joint_angles)
    mujoco.mj_step(model, data)
    ...
    mujoco.mj_step(model, data)
    data.qvel[ : 7] = curr_joint_vel
    print("----GOT VEL --- : ", data.qvel[dof_ids])
    print("----GIVEN VEL--- :", curr_joint_vel)
    difference_in_position = np.array(data.qpos[dof_ids] - joint_angles)
    # mujoco.mj_step(model, data)
    print("----FINAL VEL ----- : ", data.qvel[dof_ids])
    print("Difference in Position : ", difference_in_position)
    M_inv = M_inv[ : 7, :7] 
    C = np.zeros(model.nv)
    mujoco.mj_rne(model, data, 0, C)
    ...
    new_joint_velocities = beta_inv @ (np.array(gen_torques * time_set) + np.array(M @ curr_joint_vel))
    # new_joint_accelerations = np.array(M_inv @ (gen_torques - C[ : 7] @ curr_joint_vel))
    # new_joint_velocities = np.array(curr_joint_vel) + new_joint_accelerations * 0.02

    # mujoco.mj_rne(model, data, 0, C)
    # C = np.array(C) * 1e6
    # new_joint_accelerations = np.array(M_inv @ (gen_torques - C[ : 7] @ curr_joint_vel))
    # print("New_joint_acc :", new_joint_accelerations)
    # new_joint_velocities = np.array(curr_joint_vel) + new_joint_accelerations * 0.01
    # print("New_jnt_vel :",new_joint_velocities) 
    print("PRINTING TORQUES : ", gen_torques)
    print("PRINTING JOINT VEL : ", new_joint_velocities)
    data.qvel[ : 7] = np.zeros(7)
    mujoco.mj_step(model, data)
    print("------FINAL VEL AFTER STOPPING -----", data.qvel[dof_ids] )
...
def main() -> None:
    ...
    jac_prev, M_inv = calculate_jacobian_and_mass_matrix(model , data , arm.get_joint_states(is_radian=True, num=3)[1][0] , actuator_ids, dof_ids, site_id)
    jac_prev = jac_prev * 1000
    h = np.zeros(model.nv)
    while True : 
        step_start = time.time()
        # arm.set_mode(0)
        ...
        dx = np.array(current_waypoint - np.array(position_plus_orient[1][:3]))
        ...
        print("current_joint_velocities :",current_joint_velocities)
        ...
        jac_prev = jac1
        print("------JAC -------- : ", jac1)
        M_inv = np.array(M_inv_func) * 1e-6
        print("---------M_INV------- : ", M_inv)
        ...
        x_ddot = Kp * twist - Kd * (jac1 @ current_joint_velocities)
        mujoco.mj_rne(model, data, 0, h)
        meu = Mx @ (jac1 @ M_inv[:7, :7] @ h[:7]  -  jac_dot[ : , :7] @ arm.get_joint_states(is_radian=True, num=3)[1][1])
        tau = jac1.T @ (Mx @ x_ddot + meu)
        # Add joint task in nullspace.
        Jbar = M_inv[:7,:7] @ jac1.T @ Mx
        ddq = Kp_null * (q0[ : 7] - np.array(arm.get_joint_states(is_radian=True,num = 3)[1][0])) - Kd_null * np.array(arm.get_joint_states(is_radian=True, num= 3)[1][1])
        ...
        # break
        print("CURRENT VELOCITIES : ", current_joint_velocities)
        print("------")
        print("NEW JOINT VELOCITIES : ", dq)
        ...
        # arm.motion_enable(enable=True)
        # time.sleep(1)
        arm.vc_set_joint_velocity(dq,is_radian=True, duration = -1)
